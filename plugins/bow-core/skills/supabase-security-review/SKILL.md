...
description: Audit Supabase/backend changes (RLS, views, triggers, edge functions, SQL) for the recurring security issues the AI review gate penalises, before committing. Use when changes touch supabase/ (migrations, policies, views, edge functions) or when the user asks to "check RLS", "security review the migration", "security check", or "review supabase changes".
---

# Supabase Security Review (Security axis)
...
> Script names below (`scripts/check-security.sh`) are the default — read the
> repo's real `securityScript` from `.conventions.json` before running.

## 1. Run the project gate
```bash
bash scripts/check-security.sh --staged      # pre-commit: staged diff
# or, for the whole MR:
bash scripts/check-security.sh --branch       # diff vs origin/develop
```
...
- **rls-with-check-user-id-only** — a `WITH CHECK` whose only predicate is `auth.uid() = user_id` (FK ownership of related rows not verified). Opt out only with a `-- security: fk-not-applicable` comment when genuinely N/A.

> Any finding is BLOCKING. Fix at the printed file:line and re-run. Recipes: docs/security-review-playbook.md.
...
"AI-review" section of the pre-commit rubric in `.claude/CLAUDE.md`.
